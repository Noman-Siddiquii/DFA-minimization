/////////////////////// Group Members  \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
// 22k-4252 NOMAN AHMED SIDDIQUI
// 22K-4564 ANSH KUMAR
// 22K-4475 AFFAN JAN
// 22K-4234 SYED ASJAD ALI
#include <iostream> 
#include <string> 
#include <vector> 
#include <map> 
...
void reachable(int currentState, vector<char> alphabet) 
{
    if (currentState) 
    {
...
bool isDFAAlreadyMinimized(const vector<vector<int>>& partitions, const map<int, map<char, int>>& transitionTable, const map<int, int>& stateInPartition) {
    map<pair<int, char>, int> newStateTransitions; 

    for (const auto& [currentState, transitions] : transitionTable) {
        for (const auto& [symbol, nextState] : transitions) {
            newStateTransitions[{stateInPartition.at(currentState), symbol}] = stateInPartition.at(nextState);
        }
    }

    for (const auto& [currentState, transitions] : transitionTable) {
        for (const auto& [symbol, nextState] : transitions) {
            if (newStateTransitions[{currentState, symbol}] != stateInPartition.at(nextState)) {
                return false;
            }
        }
    }
...
int main()
{
    ...
    cout << "\nEnter the valid input symbols (Alphabets) :\n";
    cin >> temp;
    alphabet = temp;
    ...
    if (isDFAAlreadyMinimized(partitions, transitionTable, stateInPartition)) {
        cout << "\nDFA is already in its minimized form.\n\n\n";
    }
